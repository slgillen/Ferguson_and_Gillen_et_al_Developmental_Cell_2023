...
pdf(paste0(datadir,'PB_H3K27ac_ChIPseq/output_figures/BE2C_correlations.pdf'),width=500,height=400)
plot(BE2C_data)
dev.off()
...
write.table(control_consensus_1x,paste0(datadir,'PB_H3K27ac_ChIPseq/output_data/BE2C_control_Consensus_in3of5.bed'), col.names = F, row.names = F, quote = F, sep = "\t")
...
write.table(PB_consensus_1x,paste0(datadir,'PB_H3K27ac_ChIPseq/output_data/BE2C_PB_Consensus_in3of5.bed'), col.names = F, row.names = F, quote = F, sep = "\t")
...
write.table(ConsensusPeaksx,paste0(datadir,'PB_H3K27ac_ChIPseq/output_data/BE2C_Consensus_bothin3of5.bed'), col.names = F, row.names = F, quote = F, sep = "\t")
...
# plot PCA ----------------------------------------------------------------
tiff(file='PB_H3K27ac_ChIPseq/output_figures/BE2C_PCA_normalised.tiff',width=1400,height=1300,res=300)
...
write.table(data.frame(info,stringsAsFactors=FALSE),'PB_H3K27ac_ChIPseq/output_figures/info_BE2C.txt',col.names=TRUE,row.names=FALSE,quote=FALSE)
...
write.table(report, paste0("PB_H3K27ac_ChIPseq/output_data/DiffBind_BE2C_7dPBvControl_Consensus.txt"), sep = "\t", col.names = T, row.names = F, quote = F)

# plot data as volcano plot
png('PB_H3K27ac_ChIPseq/output_figures/BE2C_volcano.png',width=500,height=400)
