...
#write output table
write.csv(RNAseq_CPM,file='PBandRA_RNAseq/output_data/BE2C_PBandRA_RNAseq_CPM.csv',quote=FALSE,col.names=TRUE,row.names=FALSE)
...
nrow(kdf)
length(unique(kdf$gene_name))
kdf<-kdf[!duplicated(kdf$gene_name),]
nrow(kdf)
...
fviz_nbclust(kdf, kmeans, method='wss')#+coord_trans(ylim=c(2500,30000))
dev.off()

#visual check of the value of k choice
res.k<-hkmeans(kdf,5,hc.method='centroid')
png('kmeans_clusters_5.png',width=350,height=300)
fviz_cluster(res.k, ellipse=FALSE,labelsize=6,ggtheme = theme_classic(),geom=c('point'),pointsize=1,stand=TRUE,shape='circle')+scale_colour_manual(values = c("darkorange", "green4","mediumvioletred",'royalblue3','darkgoldenrod2','red3','grey78','grey28'))
dev.off()

#conduct kmeans clustering with k=5
res.k<-hkmeans(kdf,5,hc.method='complete')
...
hlist <- h1+h2
tiff('/PBandRA_RNAseq/output_data/Hmap_PBandRA_cluster_k5_complete.tiff',width=800,height=2400,res=300)
...
#run gene ontology analysis for cellular components
ck_go_cc<-compareCluster(gene~cluster, data=clusters[,c('gene','cluster')], fun="enrichGO", OrgDb="org.Hs.eg.db", ont='CC',pAdjustMethod='fdr', pvalueCutoff=0.05,universe=bg_universe,keyType='SYMBOL',minGSSize=20,maxGSSize=500)
dotplot(ck_go_cc, showCategory = 10)+theme_bw()+theme(panel.grid.major.x = element_blank())+scale_colour_gradient(low='forestgreen',high='gold2')

clusters$cluster<-factor(clusters$cluster,levels=c('5','4','3','2','1'))
ck_go_cc<-compareCluster(gene~clusterX, data=clusters[,c('gene','cluster')], fun="enrichGO", OrgDb="org.Hs.eg.db", ont='CC',pAdjustMethod='fdr', pvalueCutoff=0.01,universe=bg_universe,keyType='SYMBOL',minGSSize=20,maxGSSize=500)

dcc<-dotplot(ck_go_cc, showCategory = 10)+theme_bw()+theme(panel.grid.major.y = element_blank(),
...
ggsave(filename='compareCluster_CC.png',dcc,width=12,height=5)
...
dcc<-dotplot(ck_go_cc, showCategory = 10)+theme_bw()+theme(panel.grid.major.y = element_blank(),legend.position='none',plot.margin=unit(c(0,3,0,0),unit='cm'),
                                                           axis.text.x = element_text(angle = 315,vjust=0.5,hjust=0))+
  scale_colour_gradient(low='forestgreen',high='gold2')+coord_flip()+scale_y_discrete(labels = function(x) str_wrap(x, width = 50))
ggsave(filename='compareCluster_CC.png',dcc,width=12,height=4)
...
png('volcano_suvival_set1_lowisbad.png',width=500,height=350)
ggplot(data=allDE_collated_V2, aes(x=PBandRAvControl_log2FoldChange, y=-log10(PBandRAvControl_padj), col=survival_type))+geom_point()+theme_classic()+
...
png('volcano_suvival_set1_highisbad.png',width=500,height=350)
